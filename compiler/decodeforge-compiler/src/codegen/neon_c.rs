...
mod tests {
    ...
    fn emission_is_repeatable_and_free_of_host_state() {
        ...
        assert_eq!(
            ...
            "396b1b842a3e0c253e6ab876f2a838eb1173d5cd45ebc51418bc036e616fb068"
        );
        assert_eq!(first.n(), 5);
        ...
        assert!(first.source.contains("memcpy(&q_word, record + q_offset"));
        assert!(first.source.contains("vcreate_u64((uint64_t)q_word)"));
        assert!(!first.source.contains("record[q_offset + 3u] << 24"));
        assert!(first.source.contains("vcvtq_f32_s32"));
        assert!(first.source.contains("vmulq_f32"));
        assert!(first.source.contains("vaddq_f32"));
        assert!(first.source.contains("vld1q_u8(record)"));
        assert!(first.source.contains("vreinterpretq_f32_u8(scale_bytes)"));
        assert!(!first.source.contains("float scales[4]"));
        assert!(!first.source.contains("vld1q_f32(scales)"));
